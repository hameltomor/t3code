...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@xbetools/contracts",
    "--filter=@xbetools/web",
    "--filter=xbe",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=xbe"],
  "dev:web": ["run", "dev", "--filter=@xbetools/web"],
  "dev:desktop": ["run", "dev", "--filter=@xbetools/desktop", "--filter=@xbetools/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("XBECODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("XBECODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid XBECODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `XBECODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric XBECODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed XBECODE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      XBECODE_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      XBECODE_STATE_DIR: resolvedStateDir,
    };

    if (host !== undefined) {
      output.XBECODE_HOST = host;
    }

    if (authToken !== undefined) {
      output.XBECODE_AUTH_TOKEN = authToken;
    } else {
      delete output.XBECODE_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.XBECODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.XBECODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.XBECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.XBECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.XBECODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.XBECODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.XBECODE_MODE = "web";
      delete output.XBECODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read XBECODE_PORT_OFFSET/XBECODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("XBECODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "XBECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("XBECODE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.XBECODE_PORT)} webPort=${String(env.PORT)} stateDir=${String(env.XBECODE_STATE_DIR)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  stateDir: Flag.string("state-dir").pipe(
    Flag.withDescription("State directory path (forwards to XBECODE_STATE_DIR)."),
    Flag.withFallbackConfig(optionalStringConfig("XBECODE_STATE_DIR")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to XBECODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("XBECODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to XBECODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("XBECODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to XBECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("XBECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to XBECODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("XBECODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to XBECODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("XBECODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to XBECODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("XBECODE_PORT")),
  ),
