...
const makeIsolatedGitCore = (gitService: GitServiceShape) =>
  Effect.promise(async () => {
    ...
    return {
      ...
      getOriginRemoteUrl: (cwd) => core.getOriginRemoteUrl(cwd),
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "xbecode/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "xbecode/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "xbecode/tmp-working" });

        const renamed = yield* renameGitBranch({
          cwd: tmp,
          oldBranch: "xbecode/tmp-working",
          newBranch: "xbecode/feat/session",
        });

        expect(renamed.branch).toBe("xbecode/feat/session-1");
        const branches = yield* listGitBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "xbecode/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "xbecode/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("xbecode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* createGitBranch({ cwd: tmp, branch: "xbecode/feat/session" });
        yield* createGitBranch({ cwd: tmp, branch: "xbecode/feat/session-1" });
        yield* createGitBranch({ cwd: tmp, branch: "xbecode/tmp-working" });
        yield* checkoutGitBranch({ cwd: tmp, branch: "xbecode/tmp-working" });
        ...
        expect(renamed.branch).toBe("xbecode/feat/session-2");
      }),
    );
