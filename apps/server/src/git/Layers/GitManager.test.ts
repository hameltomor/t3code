...
import { GitCommandError, ForgeCliError, TextGenerationError } from "../Errors.ts";
import { type GitManagerShape } from "../Services/GitManager.ts";
import { type ForgeCliShape, type ForgeReviewRequestSummary } from "../Services/ForgeCli.ts";
...
interface FakeForgeScenario {
  provider?: "github" | "gitlab";
  findOpenSequence?: (ForgeReviewRequestSummary | null)[];
  findLatestSequence?: (ForgeReviewRequestSummary | null)[];
  defaultBranch?: string;
  /** Error returned by individual forge method calls (findOpen, findLatest, create, getDefaultBranch). */
  failWith?: ForgeCliError;
  /** Error returned by the resolver's resolve() itself (e.g. provider not detected). */
  resolveFailWith?: ForgeCliError;
}
...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("xbecode-git-remote-");
...
function createFakeForgeResolver(scenario: FakeForgeScenario = {}): {
  resolverShape: ForgeCliResolverShape;
  forgeCalls: string[];
} {
  const findOpenQueue = [...(scenario.findOpenSequence ?? [])];
  const findLatestQueue = [...(scenario.findLatestSequence ?? [])];
  const forgeCalls: string[] = [];
  const provider = scenario.provider ?? "github";

  const forgeShape: ForgeCliShape = {
    provider,

    findOpenReviewRequest: (input) => {
      forgeCalls.push(`findOpen:${input.headBranch}`);
      if (scenario.failWith) return Effect.fail(scenario.failWith);
      const result = findOpenQueue.shift() ?? null;
      return Effect.succeed(result);
    },

    findLatestReviewRequest: (input) => {
      forgeCalls.push(`findLatest:${input.headBranch}`);
      if (scenario.failWith) return Effect.fail(scenario.failWith);
      const result = findLatestQueue.shift() ?? null;
      return Effect.succeed(result);
    },

    createReviewRequest: (input) => {
      forgeCalls.push(`create:${input.baseBranch}:${input.headBranch}`);
      if (scenario.failWith) return Effect.fail(scenario.failWith);
      return Effect.void;
    },

    getDefaultBranch: (_input) => {
      forgeCalls.push("getDefaultBranch");
      if (scenario.failWith) return Effect.fail(scenario.failWith);
      return Effect.succeed(scenario.defaultBranch ?? "main");
    },
  };

  const resolverShape: ForgeCliResolverShape = {
    resolve: (_cwd) => {
      if (scenario.resolveFailWith) return Effect.fail(scenario.resolveFailWith);
      return Effect.succeed(forgeShape);
    },
  };

  return { resolverShape, forgeCalls };
}
...
function makeManager(input?: {
  forgeScenario?: FakeForgeScenario;
  textGeneration?: Partial<FakeGitTextGeneration>;
}) {
  const { resolverShape, forgeCalls } = createFakeForgeResolver(input?.forgeScenario);
  ...
  const managerLayer = Layer.mergeAll(
    Layer.succeed(ForgeCliResolver, resolverShape),
  ...
  return makeGitManager.pipe(
    ...
    Effect.map((manager) => ({ manager, forgeCalls })),
  );
}
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes PR metadata when branch already has an open PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("xbecode-git-manager-");
  ...
  it.effect("status prefers open PR when merged PR has newer updatedAt", () =>
    Effect.gen(function* () {
      ...
      const { manager } = yield* makeManager({
        forgeScenario: {
          // findLatestReviewRequest already picks open over merged,
          // so just return the open one as the latest.
          findLatestSequence: [
            {
              number: 46,
              title: "Open PR",
  ...
  it.effect("status is resilient to forge lookup failures and returns pr null", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("xbecode-git-manager-");
      yield* initRepo(repoDir);
      yield* runGit(repoDir, ["checkout", "-b", "feature/status-no-forge"]);
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      yield* runGit(repoDir, ["push", "-u", "origin", "feature/status-no-forge"]);

      const { manager } = yield* makeManager({
        forgeScenario: {
          resolveFailWith: new ForgeCliError({
            provider: "unknown",
            operation: "resolveForgeProvider",
            detail: "Could not detect forge provider from git remote.",
          }),
        },
      });

      const status = yield* manager.status({ cwd: repoDir });
      expect(status.branch).toBe("feature/status-no-forge");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const { manager, forgeCalls } = yield* makeManager({
          forgeScenario: {
            findOpenSequence: [
              null,
              {
                ...
                baseBranch: "main",
                headBranch: "feature/no-upstream-pr",
                state: "open",
                updatedAt: null,
              },
            ],
        ...
        expect(
          forgeCalls.some((call) => call.startsWith("create:main:feature/no-upstream-pr")),
        ).toBe(true);
      }),
  );
          ...
          findOpenSequence: [
            {
              ...
              baseBranch: "main",
              headBranch: "feature/existing-pr",
              state: "open",
              updatedAt: null,
            },
          ],
  ...
  it.effect("creates PR when one does not already exist", () =>
    Effect.gen(function* () {
      ...
      expect(
        forgeCalls.some((call) => call.startsWith("create:main:feature-create-pr")),
      ).toBe(true);
    }),
  );
  ...
  it.effect("surfaces missing forge CLI binary errors", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("xbecode-git-manager-");
      yield* initRepo(repoDir);
      yield* runGit(repoDir, ["checkout", "-b", "feature/forge-missing"]);
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      yield* runGit(repoDir, ["push", "-u", "origin", "feature/forge-missing"]);
      ...
      expect(errorMessage).toContain("Could not detect forge provider");
    }),
  );

  it.effect("surfaces forge auth errors with guidance", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("xbecode-git-manager-");
      yield* initRepo(repoDir);
      yield* runGit(repoDir, ["checkout", "-b", "feature/forge-auth"]);
      const remoteDir = yield* createBareRemote();
      yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
      yield* runGit(repoDir, ["push", "-u", "origin", "feature/forge-auth"]);

      const { manager } = yield* makeManager({
        forgeScenario: {
          failWith: new ForgeCliError({
            provider: "github",
            operation: "execute",
