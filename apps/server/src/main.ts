...
const CliEnvConfig = Config.all({
  mode: Config.string("XBECODE_MODE").pipe(
    Config.option,
    Config.map(
  ...
  port: Config.port("XBECODE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("XBECODE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  stateDir: Config.string("XBECODE_STATE_DIR").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("XBECODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("XBECODE_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("XBECODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("XBECODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
    ...
    Effect.gen(function* () {
      ...
      if (!isLoopbackHost(host) && !authToken) {
        return yield* new StartupError({
          message:
            "Refusing to bind a non-loopback host without WebSocket auth. Set --auth-token (or XBECODE_AUTH_TOKEN), or bind to 127.0.0.1.",
        });
      }
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("XBE Code running", {
      ...safeConfig,
...
const stateDirFlag = Flag.string("state-dir").pipe(
  Flag.withDescription("State directory path (equivalent to XBECODE_STATE_DIR)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to XBECODE_LOG_WS_EVENTS).",
  ),
  Flag.withAlias("log-ws-events"),
  Flag.optional,
);

export const xbeCli = Command.make("xbe", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  ...
  Command.withDescription("Run the XBE Code server."),
