...
/**
 * Resolve ordered prompt history entries (newest first).
 * Prefers the current thread; falls back to all same-project threads when the
 * current thread has no user messages yet.
 */
export function resolveComposerPromptHistoryEntries(
  ...
  const projectCwdById = new Map(
    input.projects.map((project) => [project.id, project.cwd] as const),
  );
...
/**
 * Navigate through prompt history. Returns the next prompt and navigation
 * state. The first ArrowUp captures the current draft; ArrowDown past index 0
 * restores it.
 */
export function navigateComposerPromptHistory(
  ...
  if (input.direction === "up") {
    ...
    if (input.navigationState.historyIndex >= input.entries.length - 1) {
      // Already at the oldest entry — clamp and stay in history mode so that
      // the key press doesn't fall through to the editor and move the caret.
      return {
        handled: true,
        nextNavigationState: input.navigationState,
        nextPrompt: input.currentPrompt,
