...
- Dev commands default `XBECODE_STATE_DIR` to `~/.xbe/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --state-dir ~/.xbe/another-dev-state`
...
- Desktop production windows load the bundled UI from `xbe://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `xbe` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `XBECODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: dev server `3774`, prod server `3775`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `XBECODE_DEV_INSTANCE`)
- Example: `XBECODE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `XBECODE_PORT_OFFSET` to a numeric offset.
