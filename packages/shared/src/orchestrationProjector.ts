...
export class OrchestrationProjectorError extends Schema.TaggedErrorClass<OrchestrationProjectorError>()(
  "OrchestrationProjectorError",
  {
  ...
  override get message(): string {
    return `Orchestration projector failed for ${this.eventType}:${this.field}: ${this.issue}`;
  }
}
...
function decodeForEvent<A>(
  ...
  field: string,
): Effect.Effect<A, OrchestrationProjectorError> {
  return Schema.decodeUnknownEffect(schema)(value).pipe(
    Effect.mapError(
      (error) =>
        new OrchestrationProjectorError({
          eventType,
          field,
...
): Effect.Effect<OrchestrationReadModel, OrchestrationProjectorError> {
  ...
  switch (event.type) {
    ...
    case "thread.created":
      return Effect.gen(function* () {
        ...
        const thread: OrchestrationThread = yield* decodeForEvent(OrchestrationThread, {
          id: payload.threadId,
    ...
    case "thread.message-sent":
      return Effect.gen(function* () {
        const payload = yield* decodeForEvent(ThreadMessageSentPayload, event.payload, event.type, "payload");
        const thread = nextBase.threads.find((entry) => entry.id === payload.threadId);
        if (!thread) {
          return nextBase;
        }

        const message: OrchestrationMessage = yield* decodeForEvent(OrchestrationMessage, {
    ...
    case "thread.context-status-set":
      return decodeForEvent(ThreadContextStatusSetPayload, event.payload, event.type, "payload").pipe(
        Effect.map((payload) => ({
          ...nextBase,
          threads: updateThread(nextBase.threads, payload.threadId, {
